(function () {
  ...
  async function loadConfigAndBookmarks() {
    ...
    applyBookmarksMask(data.config);
    bookmarks = data.bookmarks;
    renderBookmarks();
  }

  // Page-owned background mask: extracted from applySharedTheme in common.js.
  function applyBookmarksMask(config) {
  ...
  function renderBookmarks() {
    ...
    document.querySelectorAll('.bookmark-name').forEach(input => {
      input.addEventListener('input', function (e) {
        ...
        if (bookmarks[index]) bookmarks[index].name = e.target.value;
        handleInputChange();
      });
    ...
    document.querySelectorAll('.bookmark-url').forEach(input => {
      input.addEventListener('input', function (e) {
        if (isDeleting) return;
        const index = parseInt(this.dataset.index);
        if (bookmarks[index]) bookmarks[index].url = e.target.value;
        handleInputChange();
      });
      input.addEventListener('focus', function () {
        this.select();
      ...
      input.addEventListener('blur', function () {
        ...
        if (url && nameInput && (!nameInput.value || nameInput.value.trim() === '')) {
          ...
          if (bookmarks[index]) bookmarks[index].name = extractedName;
          handleInputChange();
        }
      });
    });
    ...
    document.querySelectorAll('.bookmark-tags').forEach(input => {
      input.addEventListener('input', function (e) {
        if (isDeleting) return;
        const index = parseInt(this.dataset.index);
        if (bookmarks[index]) bookmarks[index].tags = parseTags(e.target.value);
        handleInputChange();
      });
      input.addEventListener('focus', function () {
        this.select();
      ...
      input.addEventListener('blur', (e) => {
        ...
        if (e.target.value !== normalized) {
          ...
          if (bookmarks[index]) bookmarks[index].tags = parseTags(normalized);
          handleInputChange();
        }
      });
    });
